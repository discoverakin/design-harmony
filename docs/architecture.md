...
- **Horizontal card rails all go through `CardCarousel`.** There used to be two implementations — a bare `overflow-x-auto` row for "Featured this week" and an Embla carousel for "Recommended hobbies" — and they responded to different gestures, because Embla listens for a pointer *drag* and ignores wheel/trackpad events. A tester found the sideways swipe that scrolled one did nothing on the other, and separately read the un-decorated Featured row as a finished set of three cards. `CardCarousel` is now the only rail: native scroll-snap (touch, trackpad, shift+wheel, arrow keys) *plus* mouse click-drag on top, with arrows, dots and a fade over the cut-off edge that appear only when the content actually overflows. A drag that moves more than a few pixels swallows the click that follows it, so releasing over a card does not open it. Callers pass cards as children and set only a width; the rail makes each child a non-shrinking snap point. Don't add a new `overflow-x-auto` card row — the chip rows on `/events` and `/community` are filters, not rails, and stay as they are.
